...
#EFS by ctDNA/STMs combined at the MRD Window
```{r}
rm(list=ls())
setwd("~/Downloads")
circ_data <- read.csv("CLIA Testicular_GL.csv")
...
circ_data <- circ_data %>%
  mutate(ctDNA.Dynamics = case_when(
    ctDNA.MRD.Window == "NEGATIVE" & Markers.MRD.Window == "NEGATIVE" ~ "ctDNA/STMs (-)",
    (ctDNA.MRD.Window == "POSITIVE" & Markers.MRD.Window == "NEGATIVE") |
      (ctDNA.MRD.Window == "NEGATIVE" & Markers.MRD.Window == "POSITIVE") ~ "Discordant",
    ctDNA.MRD.Window == "POSITIVE" & Markers.MRD.Window == "POSITIVE" ~ "ctDNA/STMs (+)"
  ))
...
ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("blue","red","green"), title="EFS - ctDNA/STM at MRD window", ylab= "Event-Free Survival", xlab="Months from Orchiectomy", legend.labs=c("ctDNA/STMs (-)","ctDNA/STMs (+)","Discordant"), legend.title="")
summary(KM_curve, times= c(12))
circ_data$ctDNA.Dynamics <- factor(circ_data$ctDNA.Dynamics, levels=c("ctDNA/STMs (-)","Discordant","ctDNA/STMs (+)"))
circ_data$Disease.Type <- factor(circ_data$Disease.Type, levels = c("Primary", "Metastatic"))
circ_data$Histology <- factor(circ_data$Histology, levels = c("Non seminoma", "Seminoma"), labels = c("Non seminoma", "Seminoma"))
...
#Median MTM/mL at the MRD Window and by PFS status
```{r}
...
# 1. Median and range of ctDNA.MRD.MTM for POSITIVE MRD Window
positive_data <- circ_data %>% filter(ctDNA.MRD.Window == "POSITIVE")
median_positive <- median(positive_data$ctDNA.MRD.MTM, na.rm = TRUE)
range_positive <- range(positive_data$ctDNA.MRD.MTM, na.rm = TRUE)
cat("Median ctDNA.MRD.MTM in MRD-Positive group:", median_positive, "\n")
cat("Range of ctDNA.MRD.MTM in MRD-Positive group:", range_positive[1], "-", range_positive[2], "\n\n")
```
...
#EFS by ctDNA/STms combined at the Surveillance Window
```{r}
rm(list=ls())
setwd("~/Downloads")
circ_data <- read.csv("CLIA Testicular_GL.csv")
...
circ_data <- circ_data %>%
  mutate(ctDNA.Dynamics = case_when(
    ctDNA.surveillance == "NEGATIVE" & Markers.Surveillance == "NEGATIVE" ~ "ctDNA/STMs (-)",
    (ctDNA.surveillance == "POSITIVE" & Markers.Surveillance == "NEGATIVE") |
      (ctDNA.surveillance == "NEGATIVE" & Markers.Surveillance == "POSITIVE") ~ "Discordant",
    ctDNA.surveillance == "POSITIVE" & Markers.Surveillance == "POSITIVE" ~ "ctDNA/STMs (+)"
  ))
...
surv_object <-Surv(time = circ_data$PFS.months, event = circ_data$PFS.Event)
KM_curve <- survfit(surv_object ~ ctDNA.Dynamics, data = circ_data,conf.int=0.95,conf.type="log-log") 
ggsurvplot(KM_curve, data = circ_data, pval = FALSE, conf.int = FALSE, risk.table = TRUE, break.time.by=6, palette=c("blue","red","green"), title="EFS - ctDNA/STM at Surveillance window", ylab= "Event-Free Survival", xlab="Months from Orchiectomy", legend.labs=c("ctDNA/STMs (-)","ctDNA/STMs (+)","Discordant"), legend.title="")
summary(KM_curve, times= c(12))
circ_data$ctDNA.Dynamics <- factor(circ_data$ctDNA.Dynamics, levels=c("ctDNA/STMs (-)","Discordant","ctDNA/STMs (+)"))
circ_data$Disease.Type <- factor(circ_data$Disease.Type, levels = c("Primary", "Metastatic"))
circ_data$Histology <- factor(circ_data$Histology, levels = c("Non seminoma", "Seminoma"), labels = c("Non seminoma", "Seminoma"))
